...
const AromatherapyCandles: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const prod = res.data;
        const relProd = prod.filter(p => p.category === 'Candles').filter(p => p.subcategory === 'Aromatherapy');
        setProducts(relProd);
        setIsLoading(false);
      } catch (error) {
        console.error(error);
...
export default AromatherapyCandles;
